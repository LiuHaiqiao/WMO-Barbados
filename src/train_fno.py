...
class FNOLitModel(pl.LightningModule):
    ...
    def _masked_mse(pred: torch.Tensor, target: torch.Tensor,
        ...
        """MSE averaged over land pixels only. mask: (B,1,H,W) float, 1=land."""
    ...
    def _shared_step(self, batch: tuple, stage: str) -> torch.Tensor:
        ...
        total_loss = torch.zeros(1, device=x.device, dtype=x.dtype).squeeze()

        for k in range(N):
            x_k  = torch.cat([static, rain_prev, rain_curr, depth_in], dim=1)
            pred = self(x_k)
            tgt  = depth_future[:, k]          # (B, 1, P, P)

            loss_k = self._masked_mse(pred, tgt, land)
            total_loss = total_loss + loss_k

            # Physics: penalise depth increases when there is no rainfall.
            # Flood water should drain, not spontaneously grow, under dry conditions.
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            ...
            patience=self.hparams.patience, verbose=True,
        )
        return {
            'optimizer': opt,
