...
from data_loader import (
    NORM_DEM, NORM_MAN, NORM_SLOPE, NORM_RUNOFF, NORM_DEPTH,
    _read, _patch_offsets,
)
...
def load_runoff_sequence(data_dir: Path) -> list[torch.Tensor]:
    """Return list of (1, H, W) tensors, one per timestep (normalised)."""
    files = sorted((data_dir / 'runoff_timesteps').glob('runoff_hr????.00.tif'))
    return [torch.from_numpy(_read(f)[None] / NORM_RUNOFF) for f in files]
...
def patched_forward(
    ...
    x_full:     torch.Tensor,       # (6, H, W)  normalised, on CPU
    patch_size: int,
    stride:     int,
    device:     torch.device,
    ...
    for r, c in _patch_offsets(H, W, patch_size, stride):
        patch = x_full[:, r:r+p, c:c+p].unsqueeze(0).to(device)   # (1,6,P,P)
...
def run_inference(
    ...
    Parameters
    ----------
    data_dir   : simulation directory (must contain depth_timesteps/
                 and runoff_timesteps/).
    ...
    end_step   : last timestep to predict (inclusive, 0-based into runoff sequence).
    ...
    runoff_seq = load_runoff_sequence(data_dir)
    T = len(runoff_seq)
    if end_step is None:
        end_step = T - 1
    ...
    if start_step == 0:
        depth_t = torch.zeros_like(runoff_seq[0])   # cold start
    else:
    ...
    for t in tqdm(range(start_step, end_step), desc='Rolling out', unit='step'):
        runoff_t = runoff_seq[t]
        x_full   = torch.cat(
            [static, runoff_t, depth_t], dim=0
        )                                                                   # (6,H,W)
...
def save_gif(
    ...
    max_runoff: float | None = None,
    start_step: int   = 0,
) -> None:
    """
    Save a 4-panel animated GIF: Runoff | Prediction | Ground Truth | Error.

    Parameters
    ----------
    preds      : list of (1,H,W) tensors in metres
    data_dir   : simulation root (for loading GT depth + runoff sequences)
    land_mask  : (1,H,W)  1=land, 0=ocean
    gif_path   : output .gif path
    fps        : frames per second
    max_depth  : colour scale cap in metres (auto from GT peak if None)
    max_runoff : colour scale cap in mm/hr  (auto from sequence peak if None)
    ...
    runoff_seq = load_runoff_sequence(data_dir)   # list of (1,H,W) normalised

    # preds[i] → depth at t = start_step + i + 1
    #            runoff at t = start_step + i  (the forcing used at that step)
    gt_seq     = gt_seq    [start_step + 1 : start_step + 1 + len(preds)]
    runoff_seq = runoff_seq[start_step     : start_step     + len(preds)]
    ...
    if max_runoff is None:
        max_runoff = max(float(r.squeeze().max().item() * NORM_RUNOFF) for r in runoff_seq)
        max_runoff = max(max_runoff, 1.0)
    ...
    for i, (pred, gt, runoff) in enumerate(tqdm(
            zip(preds, gt_seq, runoff_seq),
            total=len(preds), desc='Rendering GIF', unit='frame')):

        p_np = _to_np(pred, land_mask)
        g_np = _to_np(gt,   land_mask)
        e_np = p_np - g_np
        r_np = runoff.squeeze().float().numpy() * NORM_RUNOFF  # → mm/hr (unmasked)
        ...
        panels = [
            (r_np, 'Runoff',       'YlGnBu', 0,        max_runoff, 'mm/hr'),
...
def parse_args():
    ...
    p.add_argument('--data_dir',   required=True,
                   help='Simulation directory (must contain depth_timesteps/, runoff_timesteps/)')
    ...
    p.add_argument('--max_runoff', type=float, default=None,
                   help='Runoff colour-scale cap in mm/hr (default: auto from sequence peak)')
...
def main():
    ...
    runoff_files = sorted((Path(args.data_dir) / 'runoff_timesteps').glob('runoff_hr????.00.tif'))
    T = len(runoff_files)
