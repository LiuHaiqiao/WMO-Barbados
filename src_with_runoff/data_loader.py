...
Each __getitem__ returns a 5-tuple for N-step rollout training:
    x              (7, P, P)       — [DEM, Manning, Pervious, Slope,
                                       Rain_t, Runoff_t, Depth_t]
    ...
    def __init__(
        ...
        for d in sample_dirs:
            ...
            if len(depth_files) != len(rain_files):
                ...
                continue
            if len(runoff_files) != len(rain_files):
                print(f'[data_loader] WARNING: skipping {d.name} — '
                      f'runoff/rain count mismatch ({len(runoff_files)} vs {len(rain_files)})')
                continue

            # valid t: need t, t+1 … t+n_steps (depth) and t+1 … t+n_steps-1 (rain/runoff)
            for t in range(0, len(depth_files) - n_steps):
                for (r, c) in offsets:
                    self.samples.append((rain_files, runoff_files, depth_files, t, r, c))

    def __len__(self) -> int:
        return len(self.samples)

    def __getitem__(self, idx: int):
        rain_files, runoff_files, depth_files, t, r, c = self.samples[idx]
        p = self.patch_size
        N = self.n_steps
        ...
        # future rain and runoff for steps 2…N  → (N-1, P, P) each
        if N > 1:
            rain_fut   = np.stack([crop(_read(rain_files[t + k]))   / NORM_RAIN
                                   for k in range(1, N)], axis=0)
            runoff_fut = np.stack([crop(_read(runoff_files[t + k])) / NORM_RUNOFF
                                   for k in range(1, N)], axis=0)
        else:
            rain_fut   = np.empty((0, p, p), dtype=np.float32)
        ...
        dynamic = torch.from_numpy(
            np.stack([rain_t, runoff_t, depth_t], axis=0)
        )
        ...
        return x, rain_future, runoff_future, depth_future, land
...
if __name__ == '__main__':
    ...
    x, rain_future, runoff_future, depth_future, land = next(iter(train_loader))
