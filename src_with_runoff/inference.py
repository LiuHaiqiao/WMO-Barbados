...
from data_loader import (
    NORM_DEM, NORM_MAN, NORM_SLOPE, NORM_RAIN, NORM_RUNOFF, NORM_DEPTH,
...
def load_rain_sequence(data_dir: Path) -> list[torch.Tensor]:
    ...
    return [torch.from_numpy(_read(f)[None] / NORM_RAIN) for f in files]
...
def patched_forward(
    ...
    for r, c in _patch_offsets(H, W, patch_size, stride):
        patch = x_full[:, r:r+p, c:c+p].unsqueeze(0).to(device)   # (1,9,P,P)
...
def run_inference(
    ...
    data_dir   : simulation directory (must contain depth_timesteps/,
                 pcpout_timesteps/, and runoff_timesteps/).
    ...
    for t in tqdm(range(start_step, end_step), desc='Rolling out', unit='step'):
        ...
        x_full   = torch.cat(
            [static, rain_t, runoff_t, depth_t], dim=0
        )                                                                   # (7,H,W)
...
def parse_args():
    ...
    p.add_argument('--data_dir',   required=True,
                   help='Simulation directory (must contain depth_timesteps/, pcpout_timesteps/, runoff_timesteps/)')
